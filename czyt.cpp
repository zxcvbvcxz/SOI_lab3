...
int main(int argc, char** argv){
	...
	if ((fd_shm_1 = shm_open (SHARED_MEM_NAME_1, O_RDONLY, 0440)) == -1)
        cout << "shm_open" << endl;

	if ((kolejka_1 = (k_kolejka*)mmap (NULL, sizeof (k_kolejka), PROT_READ, MAP_SHARED, fd_shm_1, 0)) == MAP_FAILED)
       cout << "mmap" << endl;
	
	if ((fd_shm_2 = shm_open (SHARED_MEM_NAME_2, O_RDONLY, 0440)) == -1)
        cout << "shm_open" << endl;

	if ((kolejka_2 = (k_kolejka*)mmap (NULL, sizeof (k_kolejka), PROT_READ, MAP_SHARED, fd_shm_2, 0)) == MAP_FAILED)
       cout << "mmap" << endl;
